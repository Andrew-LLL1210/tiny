...
pub fn run(gpa: std.mem.Allocator) !void {
    ...
    var server: TinyLsp = undefined;
    var handler: Handler = .{
        .gpa = gpa,
        .server = &server,
    };
    server = try TinyLsp.init(gpa, &transport, &handler);
...
pub fn getRange(span: tiny.Span, src: []const u8) lsp.types.Range {
...
pub fn getErrRange(err: tiny.Error, src: []const u8) lsp.types.Range {
    const span = switch (err) {
        .duplicate_label => |spans| spans[0],
        .builtin_label_redefinition => |sas| sas[0],
        inline else => |span| span,
    };

    const r = span.range(src);
    return .{
        .start = .{ .line = @intCast(r.start.row), .character = @intCast(r.start.col) },
        .end = .{ .line = @intCast(r.end.row), .character = @intCast(r.end.col) },
    };
}
